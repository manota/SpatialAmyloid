...
y_pred_8_month_app_coord_y_x <- read.csv('/Users/okiru/Desktop/240124 Visium_Hiroshima University/241216 prediction with c19 genes/y_pred_8_month_app_coord_y_x.csv')

# 特定の行をフィルタリング（Appのみにする）
rows_to_leave <- grep("1_1", red_8[, 1])
red_8 <- red_8[rows_to_leave, , drop = FALSE]
# 1列目のバーコードを行名に設定
rownames(app_8_coord) <- app_8_coord[, 1]
rownames(red_8) <- red_8[, 1]
rownames(y_pred_8_month_app_coord_y_x) <- y_pred_8_month_app_coord_y_x[, 1]
# 1列目を削除
app_8_coord <- app_8_coord[, -1, drop = FALSE]
red_8 <- red_8[, -1, drop = FALSE]
y_pred_8_month_app_coord_y_x <- y_pred_8_month_app_coord_y_x[, -1, drop = FALSE]
...
y_pred_8_month_app_min <- 0    ###計算枠のようなものをつくるため0を代入（初期化）
for (i in 1:nrow(y_pred_8_month_app_coord_y_x)) {
  cell_coord <- y_pred_8_month_app_coord_y_x[i, c(1, 2)]
  ...
  y_pred_8_month_app_min <- y_pred_8_month_app_min + min_distance
  print(y_pred_8_month_app_min)  # 進行状況を表示
}

print(paste("Total minimum distance:", y_pred_8_month_app_min))

##### Predictionのスポットの数と同じスポットをランダムに選んでそれを繰り返した場合の最短距離の総和

sampling_num <- nrow(y_pred_8_month_app_coord_y_x)
...
# p値の計算
p_value <- max(1/repeat_times, sum(repeat_total_mins <= y_pred_8_month_app_min) / repeat_times)
...
hist(repeat_total_mins / sampling_num / scale_factor_app_8m$tissue_lowres_scalef, xlim = c(100, 400), ylim = c(0, 3000), main = "Permutation Test Distribution", xlab = "Averaged minimum distance (pixel)", ylab = "Number of Occurrences/10000", col = "lightblue", border = "black")
abline(v = y_pred_8_month_app_min / sampling_num / scale_factor_app_8m$tissue_lowres_scalef, col = "red", lwd = 2, lty = 2)

#### Change scale pixels to microns (Ref. 10xgenomics website: scale factor)
# microns_pixels <- 65 / scale_factor_app_8m$spot_diameter_fullres
# hist(repeat_total_mins / sampling_num / scale_factor_app_8m$tissue_lowres_scalef * microns_pixels, xlim = c(100, 450), ylim = c(0, 3000), main = "Permutation Test Distribution", xlab = "Averaged minimum distance (micron)", ylab = "Number of Occurrences/10000", col = "lightblue", border = "black")
# abline(v = y_pred_8_month_app_min / sampling_num / scale_factor_app_8m$tissue_lowres_scalef * microns_pixels, col = "red", lwd = 2, lty = 2)

### 250130 modified ラベルなどのサイズを変更
microns_pixels <- 65 / scale_factor_app_8m$spot_diameter_fullres
hist(repeat_total_mins / sampling_num / scale_factor_app_8m$tissue_lowres_scalef * microns_pixels,
     xlim = c(50, 300), ylim = c(0, 2000),
